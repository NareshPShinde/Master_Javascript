...
console.log("slice(startIndex,LastIndex) :"+str.slice(1,7));//Returns a section of a string from start position to end position.allow negative indexing.  the end position is not included.if you ommit second param it will return the rest of string.
...
console.log("toUpperCase() :"+str.toUpperCase());//Converts all the alphabetic characters in a string to uppercase.
console.log("toLowerCase() :"+str.toLowerCase());//Converts all the alphabetic characters in a string to lowercase.
 
// Triming Whitespace

console.log("trim() :"+str.trim());// removes whitespace from both ends of the string and return a new string without modifying the original string.
...
console.log("padStart(maxLenght,padString) :"+str.padStart(21,'$'));//Pads the current string with a given string (possibly repeated) so that the resulting string reaches a given length. The padding is applied from the start (left) of the current string.padstring - default value is " ".
...
console.log('indexOf(SearchStrig,position) :'+str.indexOf('s',4));/*indexOf(searchString,position); The index at which to begin searching the String object. If omitted, search starts at the beginning of the string. Returns the position of the first occurrence of a substring.*/
...
console.log('startsW+ith(searchString,position) :'+str.startsWith('Master',1));// determines whether this staring begins with the character of a specified string. returning boolean value. index/position where start search. default is 0.
...
console.log("replace(seachValue,replaceValue) :"+str2.replace('program','code'));//Replaces text in a string, using a regular expression or search string
console.log("replaceAll(seachValue,replaceValue) :"+str2.replaceAll('program','code'));//Replace all instances of a substring in a string, using a regular expression or search string.
...
console.log("split(seperator,limit) :"+str.split(' '));//Split a string into substrings using the specified separator and return them as an array. seperator :A string that identifies character or characters to use in separating the string. If omitted, a single-element array containing the entire string is returned.limit : A value used to limit the number of elements returned in the array.
